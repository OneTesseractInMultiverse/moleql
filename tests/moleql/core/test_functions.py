...
class TestRemoveBlacklisted:
    def test_without_blacklist(self):
        ...
        actual: list[str] = remove_blacklisted(hql_query=INPUT_QUERY_URL_ENCODED, blacklist=None)
    ...
    def test_with_blacklist(self):
        ...
        actual: list[str] = remove_blacklisted(
            hql_query=INPUT_QUERY_URL_ENCODED, blacklist=BLACKLIST
        )
        assert expected == actual
...
class TestExtractParameterList:
    def test_with_url_encoded_hql(self):
        expected: list[str] = EXPECTED_PARAMETER_LIST
        actual: list[str] = extract_parameter_list(hql_query=INPUT_QUERY_URL_ENCODED)
        assert expected == actual

    def test_without_url_encoded_hql(self):
        expected: list[str] = EXPECTED_PARAMETER_LIST
        actual: list[str] = extract_parameter_list(hql_query=INPUT_QUERY_NOT_ENCODED)
        assert expected == actual
